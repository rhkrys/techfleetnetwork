...
interface Row { month: string; status: string; ticket_count: number }
...
export default function MonthlyReportPanel() {
  ...
  const chartData = useMemo(() => {
    ...
    for (const r of rows) {
      const key = new Date(r.month).toLocaleDateString(undefined, { year: "numeric", month: "short" });
  ...
  const statuses = useMemo(() => Array.from(new Set(rows.map((r) => r.status || "unknown"))), [rows]);
    ...
    <Card>
      ...
      <CardContent>
        {isLoading ? (
          ...
          <p className="text-sm text-muted-foreground">No data yet. Reports populate as tickets are created.</p>
        ) : (
