...
// Embedded third-party widgets (Cloudflare Turnstile, Google OAuth) reach into
// their OWN cross-origin iframe; the browser raises
// `SecurityError: Blocked a frame with origin "…" from accessing a cross-origin
// frame`. That is the widget's internals — never a Tech Fleet bug and not
// fixable in app code. Observed on /login from the Turnstile challenge iframe.
const CROSS_ORIGIN_FRAME_RE = /Blocked a frame with origin|accessing a cross-origin frame/i;
...
export function classify(value: unknown): ClassifiedError {
  ...
  // 1c. Cross-origin frame access blocked by the browser — raised by embedded
  //     third-party widgets (Turnstile captcha, Google OAuth) touching their
  //     own cross-origin iframe. Not our bug, not app-fixable. Matched by the
  //     very specific message so wrapped/renamed errors are still caught.
  if (CROSS_ORIGIN_FRAME_RE.test(err.message ?? "")) {
    return { report: false, reason: "cross_origin_frame", retriable: false };
  }
