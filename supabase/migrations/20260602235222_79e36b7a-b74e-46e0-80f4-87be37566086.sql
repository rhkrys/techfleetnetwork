...
BEGIN
  ...
  FROM public.audit_log WHERE timestamp >= now() - INTERVAL '7 days';
  ...
  FROM (
    SELECT actor_id, count(*) AS cnt FROM public.audit_log
    WHERE timestamp >= now() - INTERVAL '30 days'
  ...
  WHERE a.timestamp >= now() - INTERVAL '30 days'
    ...
    AND a.timestamp - p.created_at < INTERVAL '5 minutes';
  INSERT INTO public.refactor_kpi_daily (snapshot_date, metric_key, metric_value, metric_unit, window_label)
  ...
  SELECT count(DISTINCT actor_id) INTO v_chunk_brick FROM public.audit_log
  WHERE timestamp >= now() - INTERVAL '7 days'
...
END;
$$;
REVOKE ALL ON FUNCTION public.snapshot_refactor_kpis() FROM PUBLIC, anon;
