-- PR 9 (email rearchitecture): remove the triage-digest feature (owner decision).
--
-- 1. Unschedule its cron ('triage-digest-daily'); the edge function triage-digest-builder is deleted.
-- 2. CREATE OR REPLACE environment_readiness() to drop 'triage-digest-daily' from the expected-cron
--    coverage list — otherwise System Health would report the now-removed job as missing (a false
--    regression). Reproduced verbatim from 20260809120200 with that ONE VALUES row removed.
--
-- The triage-error and notify-critical-fix functions + the triage-critical-push cron are UNAFFECTED.
