-- 1. discord_linked_at column + trigger
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS discord_linked_at timestamptz;

UPDATE public.profiles p
SET discord_linked_at = COALESCE(
  (SELECT MIN(b.awarded_at) FROM public.badges_awarded b
   WHERE b.user_id = p.user_id AND b.badge_code = 'discord_linked'),
  p.created_at
)
...
CREATE OR REPLACE FUNCTION public.set_discord_linked_at()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp AS $$
BEGIN
  IF (NEW.discord_user_id IS NOT NULL AND NEW.discord_user_id <> '')
     AND (TG_OP = 'INSERT' OR OLD.discord_user_id IS NULL OR OLD.discord_user_id = '')
     AND NEW.discord_linked_at IS NULL
  THEN
    NEW.discord_linked_at := now();
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS trg_set_discord_linked_at ON public.profiles;
CREATE TRIGGER trg_set_discord_linked_at
BEFORE INSERT OR UPDATE OF discord_user_id ON public.profiles
FOR EACH ROW
EXECUTE FUNCTION public.set_discord_linked_at();
...
-- 2. Rewrite recompute_all_stats() with immutable discord timestamp + new metrics
...
DECLARE
  ...
  v_apps_total bigint; v_badges_total bigint;
  ...
  v_pw_apps bigint; v_pw_badges bigint;
BEGIN
...
-- 3. Rewrite get_network_stats to expose all-tier totals + onboarding breakdown
...
-- 4. Refresh snapshot immediately so UI reflects the fix on next load
