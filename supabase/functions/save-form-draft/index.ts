...
function validate(body: DraftBody): { ok: true; draftKey: string; schemaVersion: number; payload: unknown } | { ok: false; error: string } {
  if (typeof body.draft_key !== "string" || body.draft_key.length === 0 || body.draft_key.length > MAX_KEY_LEN) {
    return { ok: false, error: "draft_key must be a non-empty string ≤200 chars" };
  }
  if (typeof body.schema_version !== "number" || !Number.isInteger(body.schema_version) || body.schema_version < 1) {
