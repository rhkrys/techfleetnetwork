...
Deno.serve(
  withAuditWrapper("login-with-captcha", async (req) => {
    ...
    try {
      ...
      if (!secret || !supabaseUrl || !anonKey) {
        ...
        return exit(503, "config_missing", {
      ...
      // eslint-disable-next-line no-console
      console.log(
        `[login-with-captcha] domain req=${requestId} dom=${maskDomain(domain)} branch=${domainCheck.branch} valid=${domainCheck.valid}`
      );
      if (!domainCheck.valid) {
        log.warn("validate", `Rejected login with non-existent email domain [${requestId}]`, {
          requestId,
          branch: domainCheck.branch,
        });
      ...
      async function verifyCaptcha(secretKey: string) {
        ...
        verifyForm.set("response", captchaToken);
      ...
      if (!authResponse.ok) {
        log.warn("auth", `Password login rejected after CAPTCHA [${requestId}]`, {
          requestId,
        ...
        if (authResponse.status === 429) {
          return exit(
            ...
            {
              error:
                "Too many rapid auth attempts. Complete the human verification before trying again.",
              code: "AUTH_THROTTLE_CAPTCHA_REQUIRED",
            },
      ...
      // eslint-disable-next-line no-console
      console.log(
        `[login-with-captcha] EXIT req=${requestId} status=${exitStatus} branch=${branch} duration_ms=${Date.now() - startedAt}`
      );
      ...
      try {
        ...
        const body = (parsed?.success ? parsed.data : null) as {
          email?: string;
          attemptId?: string;
        } | null;
        if (supabaseUrl && serviceKey && body?.attemptId) {
          const ip = clientIp(req) ?? null;
          // `branch` is mutated via the `exit` closure, which TS's control-flow
          // analysis can't track — it over-narrows `branch` to its initializer
          // here. Widen back to the declared union for the outcome mapping.
          const outcomeBranch = branch as LoginBranch;
          await fetch(`${supabaseUrl}/rest/v1/rpc/record_login_event`, {
            ...
            body: JSON.stringify({
              ...
              p_outcome:
                outcomeBranch === "ok"
                  ? "edge_entered"
                  : outcomeBranch === "captcha_fail"
                    ? "captcha_failed"
                    : outcomeBranch === "domain_reject"
                      ? "domain_reject"
                      : outcomeBranch === "throttle"
                        ? "auth_throttle"
                        : outcomeBranch === "token_4xx"
                          ? "invalid_credentials"
                          : outcomeBranch === "token_5xx"
                            ? "server_error"
                            : outcomeBranch === "validate_fail"
                              ? "unknown"
                              : outcomeBranch === "config_missing"
                                ? "server_error"
                                : "edge_entered",
              p_branch: outcomeBranch,
