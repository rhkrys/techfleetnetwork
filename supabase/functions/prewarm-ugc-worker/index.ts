...
async function translate(text: string, locale: string, format: string): Promise<{ ok: boolean; out?: string; reason?: string }> {
...
function qa(source: string, translated: string, locale: string): { pass: boolean; gate?: string; report: Record<string, unknown> } {
  const report: Record<string, unknown> = { locale };
  // Gate 1: non-empty, length sanity
  if (!translated || translated.length === 0) return { pass: false, gate: "structural", report: { ...report, why: "empty" } };
  if (translated.length > source.length * 5 + 100) return { pass: false, gate: "structural", report: { ...report, why: "length_blowup" } };
  ...
  if (srcPh.join("|") !== outPh.join("|")) return { pass: false, gate: "placeholders", report: { ...report, src: srcPh, out: outPh } };
  // Gate 3: brand lock
  ...
  if (translated.trim() === source.trim()) return { pass: false, gate: "language", report: { ...report, why: "no_change" } };
