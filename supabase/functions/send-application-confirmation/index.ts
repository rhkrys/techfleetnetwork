...
async function loadRecipientContext(
  ...
  if (row.kind === 'project' && row.project_id) {
    ...
    if (proj) {
      ...
      projectName = [clientName, (proj.friendly_name as string) || null].filter(Boolean).join(' — ') || null
    }
  }
...
Deno.serve(async (req) => {
  ...
  if (serviceAuth.ok) {
    ...
    return new Response(JSON.stringify({ mode: 'sweep', sent, failed, scanned: rows?.length ?? 0 }), {
      status: 200,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    })
  }
