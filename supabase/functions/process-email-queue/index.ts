...
Deno.serve(
  withAuditWrapper("process-email-queue", async (req) => {
    ...
    const windowAgoIso = new Date(
      Date.now() - perRecipientWindowHours * 60 * 60 * 1000
    ).toISOString();
    ...
    for (const queue of ["auth_emails", "transactional_emails", "bulk_emails"]) {
      ...
      if (
        (consecutive[queue] ?? 0) > 0 &&
        (!cooldownUntil[queue] || new Date(cooldownUntil[queue] as string) <= new Date())
      ) {
        try {
          await supabase
            .from("email_send_state")
          ...
          if (isDeliverabilityBulk && !bypassCap && perRecipientMax > 0) {
            ...
            if ((recentToRecipient ?? 0) >= perRecipientMax) {
              ...
              await supabase.rpc("write_audit_log", {
                ...
                p_changed_fields: [
                  `template:${labelStr || queue}`,
                  `cap:${perRecipientMax}`,
                  `window_hours:${perRecipientWindowHours}`,
                ],
              });
        ...
        try {
          ...
          if ((consecutive[queue] ?? 0) > 0) {
            try {
              ...
              console.warn("Failed to reset consecutive counter — non-fatal", {
                err: String(resetErr),
              });
            }
          }
          ...
          if (isRateLimited(error)) {
            ...
            const offenderLane =
              isWorkspaceQuota && lastSentLane && lastSentLane !== queue ? lastSentLane : queue;
