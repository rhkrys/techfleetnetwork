...
function readJsSet() {
  const src = readFileSync(
    join(ROOT, "src/services/error-reporter.service.ts"),
    "utf8",
  );
  const block = src.match(
    /NON_ACTIONABLE_EVENT_TYPES[^=]*=\s*new Set\(\[([\s\S]*?)\]\)/,
  );
  if (!block) {
...
function readSqlSet() {
  ...
  console.error(
    "[triage-parity] could not find is_actionable_event_type migration",
  );
  process.exit(2);
}
